...
function getEnvVar(key) {
    var _a, _b;
    var input = (_a = process.env["INPUT_".concat(key.replace(/ /g, '_').toUpperCase())]) === null || _a === void 0 ? void 0 : _a.trimEnd();
    var raw = (_b = process.env[key]) === null || _b === void 0 ? void 0 : _b.trimEnd();
...
function getInput(input) {
    ...
    if (!types_1.VALID_TYPES.has((_a = options.type) !== null && _a !== void 0 ? _a : String)) {
        if (options.type instanceof Array) {
            ...
            for (var _i = 0, _g = options.type; _i < _g.length; _i++) {
                var type = _g[_i];
    ...
    if (options.type instanceof Array) {
        var parsedArray = (_c = parsed) !== null && _c !== void 0 ? _c : [];
        var tmp = [];
        if (options.default instanceof Array) {
            var defaultArray = (_d = options.default) !== null && _d !== void 0 ? _d : [];
            var typeLength = options.type.length === 1 ? Math.max(defaultArray.length, parsedArray.length) : options.type.length;
            for (var index = 0; index < typeLength; index++) {
                tmp.push((_e = parsedArray[index]) !== null && _e !== void 0 ? _e : defaultArray[index]);
            }
            parsed = tmp;
        }
        else if (typeof val === 'string') {
            var typeLength = options.type.length === 1 ? parsedArray.length : options.type.length;
            for (var index = 0; index < typeLength; index++) {
                tmp.push((_f = parsedArray[index]) !== null && _f !== void 0 ? _f : options.default);
            }
            parsed = tmp;
        }
    }
...
function getInputs(inputs) {
    ...
    for (var _i = 0, _b = Object.entries(inputs); _i < _b.length; _i++) {
        ...
        inputs[k] = getInput((_a = v) !== null && _a !== void 0 ? _a : k);
    }
    return inputs;
}
